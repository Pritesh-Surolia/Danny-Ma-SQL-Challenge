...
-- Creation of the Database
	...
	VALUES
	 ...
	 The last INSERT Statement does not work here because we are trying to insert more than 1000 rows in the subscriptions table and SQL SERVER has a limit of 
	 1000 row values in the INSERT statement. So as a workaround, I will do the following.

	 1. Create the 'subscriptions' table in MySQL Workbench 
	 2. Insert all the row values (MySQL has no such limit on inserting rows)
	 3. Display all the rows with a quick SELECT query
	 4. Export the output as a CSV file
	 5. Import the CSV file in the 'foodie_fi' database here as the 'subscriptions' table

	 And Voila! We'll have our database ready with all the required tables.

	 */

	 -- So first I need to drop the empty 'subscriptions' table I just created.

	 DROP TABLE dbo.subscriptions;

	 -- Now that we do not have a table with the same name in our database, I will go to the databse 'foodie_fi' --> 'Tasks' --> 'Import Flat File'  --> Choose the specifics of the table 
	 -- And a new table with the name 'subscriptions' and all the rows will be created in the database.

	 -- Let's check if we imported the table successfully
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	 ...
	 A. Customer Journey: Based off the 8 sample customers provided in the sample from the subscriptions table, write a brief description about each customer’s 
	    onboarding journey.
	 
	 */

	 -- Solution: 
	 -- Approach: I joined the 2 given tables and selected only the required sample of customers. Additionally, I added a derived column 'days_to_upgarde' by calculating 
	 -- the difference between start_date of each plan and the immediately previous plan for each customer using the window function LAG(). 
	 -- This column provides further insights into the customer's onboarding journey.

	 SELECT s.customer_id, p.plan_name,start_date,DATEDIFF(DAY,LAG(s.start_date,1) OVER(PARTITION BY customer_id ORDER BY start_date),s.start_date) AS days_to_upgrade
	 ...
	 SELECT DATEPART(MONTH,start_date) AS Month, COUNT(DISTINCT customer_id) AS number_of_trials_taken --I could use the MONTH() Function as well to get the month from the start_date
	 FROM subscriptions
	 WHERE plan_id = 0
	 ...
	 SELECT plan_name, COUNT(s.plan_id) AS count_of_occurrence
	 FROM subscriptions s
	 JOIN plans p
	 ON s.plan_id = p.plan_id
	 WHERE YEAR(start_date) > '2020'
	 ...
	 SELECT plan_name,COUNT(DISTINCT customer_id) AS churned_customers,
	 ...
	 -- Using CTE
	 -- Approach: I have created a CTE for the two required counts total customers and churned customers and then calculated the churn percentage using the 2 counts.
	...
	SELECT COUNT(customer_id) AS churn_after_trial_count,100*COUNT(customer_id)/(SELECT COUNT(DISTINCT customer_id) FROM subscriptions) AS churn_percentage
	FROM dates_cte
	...
	-- LEAD() is a window function that gives the record following the current record. It takes 3 arguments - the field you wanna obtain,
	-- the number of records following the current record and the default value in case of no following record (by default it will show NULL)
	...
	SELECT next_plan,COUNT(customer_id) AS customer_count,
	...
	/* Q9. How many days on average does it take for a customer to upgrade to an annual plan from the day they join Foodie-Fi?  */


	-- Approach: This one is similar to the 1st approach in Q5 where we calculated the churned customers count.
	-- Get the two dates, joining_day and annual_join_day in two separate derived tables.
	-- Then, join the two tables, calculate the difference between the two dates and then find the average. 

	SELECT ROUND(AVG(CONVERT(FLOAT,DATEDIFF(DAY,joining_day,annual_plan_day))),2) AS avg_days --Here, the calculated difference is INT, so the AVG() returns the INT part only.
	                                                                --Converting the calculated difference into FLOAT returns the fractional part of the value as well.
	FROM 
	(SELECT customer_id, start_date AS annual_plan_day
	FROM subscriptions
	WHERE plan_id = 3) AS A
	JOIN 
	(SELECT customer_id, start_date AS joining_day
	FROM subscriptions
	WHERE plan_id = 0 AND customer_id IN (SELECT customer_id FROM subscriptions WHERE plan_id = 3)
	) AS B
	ON A.customer_id = B.customer_id

	-- Approach2:
	-- So instead of getting the joining_day in a separate derived table, we can join the 'subscriptions' table with annual_plan_day_cte for rows where plan_id = 0. 
	-- This will return the trial_start_date that will be the joining day and the annual_plan_day in the combined table.
	-- Then, calculate the average of difference between the 2 dates like we did in the previous approach.
	...
	SELECT ROUND(AVG(CONVERT(FLOAT,DATEDIFF(DAY,start_date,annual_plan_day))),2) AS avg_days
	FROM subscriptions s
	...
	-- Approach: We create 3 CTEs. First is trial_date_cte, Second is annual_plan_date_cte. We use these two CTEs to create a third CTE 'buckets' in which we divide 
	-- the days_of_upgrade  for different customers into 12 buckets, each of 30 days. 
	...
	DATEDIFF(DAY,trial_date,annual_plan_date)/30 + 1 AS bucket --Dividing the days difference by 30 returns the integer part of the quotient that is 1 less than the bucket it is falling in. Adding 1 returns the bucket number the days difference falls in.
